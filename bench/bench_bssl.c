...
int main_dhkem_bssl(void)
{
    ...
    if (EVP_HPKE_KEY_public_key(&key, pkR.data, &pkR.len,
    ...
    if (EVP_HPKE_CTX_open(rctx, clear, &clearlen, LBUFSIZE, ct, ctlen, NULL,
                          0) <= 0)
    {
...
int main_auth_dhkem_bssl(void)
{
    ...
    if (EVP_HPKE_KEY_public_key(&skey, pkS.data, &pkS.len,
                                EVP_HPKE_MAX_PUBLIC_KEY_LENGTH) <= 0)
    {
        handle_errors("EVP_HPKE_KEY_public_key failed");
    }
    ...
    if (EVP_HPKE_KEY_public_key(&rkey, pkR.data, &pkR.len,
                                EVP_HPKE_MAX_PUBLIC_KEY_LENGTH) <= 0)
    {
        handle_errors("EVP_HPKE_KEY_public_key failed");
    }
